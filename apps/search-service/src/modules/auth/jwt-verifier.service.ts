 ...
 * Shared as a base64-encoded SPKI PEM via AUTH_PUBLIC_KEY_B64 — same env-var name
 * as catalog-service so a single key rollout updates every consumer.
 */
@Injectable()
export class JwtVerifierService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    if (!b64) {
      this.logger.warn(
        'AUTH_PUBLIC_KEY_B64 is not set — admin endpoints will reject all tokens',
      );
      return;
    }
