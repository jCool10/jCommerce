 ...
 * Shared with every other consumer via `AUTH_PUBLIC_KEY_B64` — a single key
 * rotation updates the whole fleet.
 */
@Injectable()
export class JwtVerifierService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    if (!b64) {
      this.logger.warn(
        'AUTH_PUBLIC_KEY_B64 is not set — every protected route will reject',
      );
      return;
    }
  ...
  async verify(token: string): Promise<VerifiedClaims> {
    ...
    // Pin iss/aud so tokens signed for a different audience (e.g. a sibling
    // tenant or stolen from a non-jcool issuer) are rejected at verify time.
