...
export class OutboxPollerCron {
  ...
  async drain(): Promise<void> {
    ...
    try {
      ...
      setOutboxPending('catalog', Math.max(0, total - published));

      if (total > 0 && published < total) {
        this.logger.warn(
          `catalog outbox: ${published}/${total} published; ${total - published} will retry`,
        );
      }
    } catch (error) {
      this.logger.error(
        `catalog outbox drain failed: ${(error as Error).message}`,
      );
    } finally {
      this.busy = false;
    }
